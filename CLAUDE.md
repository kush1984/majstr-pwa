...
- `npm run test` — **Vitest** unit/component layer (jsdom, no backend needed). Fast pre-commit check. Tests are co-located as `src/**/*.test.ts(x)` and excluded from `tsc -b` (the build). `npm run test:watch` for watch mode. Covers the retry policy (`lib/queryRetry`), the Sentry scrubbers (`lib/sentry`), and the `ErrorBoundary`.
- `npm run test:e2e` — Playwright, backend-backed. Two specs: `smoke.spec.ts` (contractor happy path: register → new estimate → add item → share) and `reliability.spec.ts` (backend-down → friendly error screen + retry recovers). **Requires the backend on `:8080`** (it checks reachability and fails fast otherwise); the Vite dev server is auto-started, or reused if already on 5173. `npm run test:e2e:ui` opens the debug UI. (`reliability.spec.ts` runs first alphabetically, so it eats the cold-Vite compile — its timeouts are deliberately generous.)
